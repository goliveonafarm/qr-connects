...
const ParticipantResponseCardBody = ({ response, getParticipantResponsesWithEvents, deleteParticipantResponse }) => {
  ...
  return (
    ...
    >
      <div>
        <div className="font-bold text-lg">Response Info-</div>
        <div>Active: {response.active ? `True` : `False`}</div>
        <div>Form type: {response.formType}</div>
        <div>QR Shareable: {response.shareable ? `True` : `False`}</div>
        <div>
          Share Results w/ Particip: {response.shareResults ? `True` : `False`}
        </div>
        <div>
          Hide names on results from Particip:{" "}
          {response.privateResults ? `True` : `False`}
        </div>
      </div>
      <div>
