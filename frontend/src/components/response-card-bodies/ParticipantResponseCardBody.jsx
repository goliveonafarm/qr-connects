...
const ParticipantResponseCardBody = ({ response, getParticipantResponsesWithEvents, deleteParticipantResponse }) => {
  const { updatingResponse, updateResponse } = useUpdateParticipantResponse();
  //next goal for this component is to make the participantResponse an object, 
  //and update the response.respnseData
  //this would require changing the backend responseData model to be an object instead of array
  ...
  const userSubmission = response.responseData[0] ?  convertToBool(response.responseData[0]) : null;
    ...
    >
      ...
      <div className="flex">
        <div className="mr-auto ml-auto">
          <button onClick={() => handleUpdateResponse([true], response._id)}>
            <svg
              height={"50"}
              width={"50"}
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 512 512"
              className={userSubmission && "fill-success"}
            >
              <path 
        ...
        <div className="mr-auto ml-auto">
          <button onClick={() => handleUpdateResponse([false], response._id)}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="50"
              width="50"
